...
- 이 부분은 쿠버네티스로 ip를 관리하는 법과 연관이 된다. ***쿠커네티스 네트워크에서는 특정 service에 IP 주소가 아닌 서비스의 이름으로 접근할 수 있다!!!*** 따라서 mysql 서비스의 주소가 아닌 서비스 이름인 mysql을 적어준 것. 물론 이 방법은 외부에서는 불가능하고 쿠버네티스 안의 
